...
export default function AnalyticsDashboard() {
  return (
    <motion.div
      ...
      transition={{ y: { duration: 5, repeat: Infinity, ease: "easeInOut" }, opacity: { duration: 1.5 }, scale: {duration: 1.5} }}
      className="relative w-full max-w-lg mx-auto"
    >
      <div className="rounded-3xl border border-[#0891B2]/30 bg-[#0D1235]/80 backdrop-blur-xl p-5 shadow-2xl"
        style={{ boxShadow: "0 0 60px rgba(8,145,178,0.15), 0 0 120px rgba(59,130,246,0.08)" }}>
        <div className="mb-4 flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-slate-400">Channel Analytics</p>
        ...
        <div className="grid gap-3 sm:grid-cols-3 mb-4">
          {[
            { label: "Subscribers", value: "124.8K", change: "+18.3%", color: "#0891B2" },
            { label: "Views (30d)", value: "2.4M", change: "+31.2%", color: "#3B82F6" },
            { label: "Watch Time", value: "68%", change: "+12.1%", color: "#F59E0B" },
          ].map((stat) => (
            <div key={stat.label} className="rounded-2xl bg-[#0A0E27]/80 border border-white/5 p-3">
            ...
            {bars.map((value, index) => (
              <motion.div
                ...
                style={{
                  originY: 1,
                  height: `${value}%`,
                  background: index === bars.length - 1
        ...
        <div className="flex items-center justify-between border-t border-white/5 pt-3">
          <div className="flex items-center gap-2 text-slate-400">
            ...
            <span className="text-xs">Avg CTR: <span className="font-semibold text-white">7.4%</span></span>
          </div>
