...
export default function GmailCredentialManager({
  ...
  const [related, setRelated] = useState<
    Array<{ id: string; service: string; username: string; password: string }>
  >([]);
  ...
  async function save() {
    ...
    try {
      ...
      const { account } = editing
        ? await apiPatch<{ account: ClientGmailRow }>(
            `/api/admin/gmail-accounts/${editing.id}`,
            input,
          )
        : await apiPost<{ account: ClientGmailRow }>(
      ...
      close();
      router.refresh();
      toast.success(t(created ? "created" : "updated"));
    } catch (cause) {
      reportError(cause);
    } finally {
      setBusy(false);
      ...
      <CardContent className="space-y-3">
        ...
        {rows.map((row) => (
          <div key={row.id} className="space-y-2 rounded border p-3">
            ...
            <p>{t(row.has_secret ? "configured" : "rotationRequired")}</p>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                variant="outline"
                onClick={() => {
                  if (window.confirm(t("deleteConfirm"))) void remove(row.id);
                }}
                className="cursor-pointer"
              >
                {t("delete")}
        ...
        >
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {t(editing ? "replaceTitle" : "addTitle")}
              </DialogTitle>
            ...
            <Button
              disabled={
                ...
                !email ||
                !password ||
                related.some((r) => !r.service || !r.username || !r.password)
              }
              onClick={() => void save()}
            >
              {t(busy ? "saving" : "save")}
