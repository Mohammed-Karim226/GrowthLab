...
export default function GmailPasswordRevealDialog({
  account,
  onClose,
}: {
  ...
  const [credentials, setCredentials] =
    useState<RevealedGmailCredentials | null>(null);
  const [errorKey, setErrorKey] = useState<string | null>(null);

  useEffect(() => {
    const controller = new AbortController();
    void apiFetch<{ credentials: RevealedGmailCredentials }>(
      `/api/admin/gmail-accounts/${account.id}/reveal`,
      {
        method: "POST",
        cache: "no-store",
      ...
      .catch((cause: unknown) => {
        if (!controller.signal.aborted)
          setErrorKey(
            cause instanceof ApiRequestError ? cause.errorKey : "serverError",
          );
      });
    ...
    >
      <DialogContent>
        ...
        {!credentials && !errorKey && (
          <p role="status">{t("loadingPasswords")}</p>
        )}
        ...
        {credentials && (
          <div className="space-y-4">
            ...
            {credentials.relatedAccounts.map((related) => (
              <label key={related.id} className="block space-y-2">
                <span>
                  <bdi>{related.service}</bdi> / <bdi>{related.username}</bdi>
                </span>
                <Input
                  dir="ltr"
                  type="text"
