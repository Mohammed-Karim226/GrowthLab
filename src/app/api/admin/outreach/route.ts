...
export const GET = withAdmin("listOutreach", async (_session, request) => {
  ...
  const pageSize = Math.min(
    50,
    Math.max(25, Number(url.searchParams.get("pageSize") || "25")),
  );
  ...
  return apiOk({
    ...
    pagination: {
      ...
      hasMore:
        from + (messagesResult.data?.length || 0) < (messagesResult.count || 0),
    },
...
export const POST = withAdmin("createOutreach", async (session, request) => {
  ...
  await writeAuditLog(db, {
    ...
    action: `outreach.${input.kind}.created`,
    entity_type: `outreach_${input.kind}`,
...
export const PATCH = withAdmin(
  ...
  async (session, request) => {
    ...
    if ("kind" in parsed.data && parsed.data.kind === "message") {
      ...
      await writeAuditLog(db, {
        actor_id: session.userId,
        action: "outreach.message.updated",
    ...
    const values = {
      ...
      last_event: `Marked ${parsed.data.status.replace("_", " ")}`,
      ...(parsed.data.status === "sent" ? { sent_at: now } : {}),
      ...(parsed.data.status === "replied" ? { replied_at: now } : {}),
    };
    const db = await createClient();
    const { data, error } = await db
      .from("outreach_messages")
    ...
    await writeAuditLog(db, {
      actor_id: session.userId,
      action: "outreach.message.status_changed",
      entity_type: "outreach_message",
      entity_id: data.id,
...
export const DELETE = withAdmin("deleteOutreach", async (session, request) => {
  ...
  if ("clearAll" in parsed.data && parsed.data.clearAll) {
    ...
    await writeAuditLog(db, {
      actor_id: session.userId,
      action: "outreach.messages.cleared",
      entity_type: "outreach_message",
      metadata: { scope: "all" },
    });
  ...
  await writeAuditLog(db, {
    actor_id: session.userId,
    action: "outreach.message.deleted",
    entity_type: "outreach_message",
    entity_id: parsed.data.id,
  });
  return apiOk({ deleted: parsed.data.id });
